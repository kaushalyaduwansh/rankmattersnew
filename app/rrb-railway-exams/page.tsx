...
import { ArrowRight, BarChart3, TrendingUp, TrainFront, Gauge } from 'lucide-react';
import Header from '@/components/header'; 
import Footer from '@/components/footer';

// --- 1. METADATA FOR SEO ---
export const metadata = {
  title: "RRB Railway Rank Predictor | RankMatters",
  description: "Check normalized marks and zone-wise ranks for RRB NTPC, Group D, ALP, Technician and other Railway exams.",
};

// --- 2. FETCH ONLY RRB EXAMS ---
async function getRRBExams() {
  const exams = await db.select()
    .from(recentExams)
    .where(eq(recentExams.type, 'RRB')) // <--- The Key Filter
    .orderBy(desc(recentExams.createdAt));
  return exams;
}

export default async function RRBPage() {
  const exams = await getRRBExams();

  return (
    <div className="min-h-screen bg-[#FAFAFA] font-sans text-slate-900 selection:bg-green-100 flex flex-col">
      ...
      <main className="pt-24 md:pt-36 flex-grow container mx-auto px-4 md:px-6 max-w-6xl">
        ...
        {/* --- RRB SPECIFIC HERO SECTION --- */}
        <div className="text-center mb-12 md:mb-20 space-y-4 md:space-y-6">
            {/* Green Badge for Railway */}
            <div className="inline-flex items-center gap-2 px-3 py-1 md:px-4 md:py-1.5 rounded-full bg-green-50 border border-green-100 text-green-700 text-[10px] md:text-xs font-bold uppercase tracking-widest animate-in fade-in zoom-in duration-500">
                <TrainFront className="w-3 h-3 text-green-600" /> 
            ...
            <h1 className="text-3xl md:text-5xl lg:text-6xl font-extrabold tracking-tight text-slate-900 leading-[1.1] max-w-4xl mx-auto">
                RRB Rank Predictor <br className="hidden md:block" />
            ...
            <p className="text-sm md:text-lg text-slate-500 max-w-2xl mx-auto leading-relaxed">
                Analyze your performance with Zone-wise ranking. 
                Get accurate normalization based on your shift difficulty.
            </p>
        </div>
                ...
                {exams.length === 0 && (
                    <div className="col-span-full flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-slate-200">
                        <div className="w-16 h-16 bg-green-50 rounded-full flex items-center justify-center mb-4">
                            <TrainFront className="text-green-400 w-8 h-8" />
                        </div>
                        <h3 className="text-lg font-bold text-slate-700">No Active RRB Keys</h3>
                        <p className="text-slate-500 text-sm mt-1">
                            Answer keys for upcoming Railway exams will appear here.
                        </p>
                    </div>
                )}
            </div>
...
// --- REUSED CARD COMPONENT (Customized for RRB - Green Tint) ---
    ...
    return (
        <Link href={data.url} className="block h-full group">
            <div className="relative h-full bg-white rounded-xl p-4 border border-slate-100 shadow-[0_2px_8px_-4px_rgba(0,0,0,0.05)] hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.1)] hover:-translate-y-1 transition-all duration-300 ease-out overflow-hidden flex flex-col justify-between">
                ...
                {/* Glow Effects (Green tint for Railway) */}
                ...
                <div className="relative z-10">
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-center">
                            ...
                            <h3 className="font-bold text-slate-900 text-base leading-tight truncate pr-1 group-hover:text-green-600 transition-colors">
                                {data.examName}
                            </h3>
                ...
                <div className="relative z-10 mt-4 pt-3 border-t border-slate-50 flex items-center justify-between">
                    <span className="text-[10px] font-bold px-2 py-1 rounded bg-green-50 text-green-700 uppercase tracking-wider border border-green-100 transition-all duration-300">
                        {data.type}
                    </span>

                    <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-green-50 text-green-700 text-[11px] font-bold group-hover:bg-green-600 group-hover:text-white transition-all duration-300">
                        Analyze
