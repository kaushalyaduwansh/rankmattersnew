...
import { ArrowRight, BarChart3, TrendingUp, Sparkles, BookOpen } from 'lucide-react';
import Header from '@/components/header'; // Adjust path if needed (e.g. ../header)
import Footer from '@/components/footer'; // Adjust path if needed

// --- 1. METADATA FOR SEO ---
export const metadata = {
  title: "SSC Rank Predictor & Answer Keys | RankMatters",
  description: "Check normalized marks and ranks for SSC CGL, CHSL, MTS, CPO and other Staff Selection Commission exams.",
};

// --- 2. FETCH ONLY SSC EXAMS ---
async function getSSCExams() {
  const exams = await db.select()
    .from(recentExams)
    .where(eq(recentExams.type, 'SSC')) // <--- The Key Filter
...
export default async function SSCPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] font-sans text-slate-900 selection:bg-orange-100 flex flex-col">
      ...
      <main className="pt-24 md:pt-36 flex-grow container mx-auto px-4 md:px-6 max-w-6xl">
        ...
        {/* --- SSC SPECIFIC HERO SECTION --- */}
        <div className="text-center mb-12 md:mb-20 space-y-4 md:space-y-6">
            {/* Orange Badge for SSC */}
            <div className="inline-flex items-center gap-2 px-3 py-1 md:px-4 md:py-1.5 rounded-full bg-orange-50 border border-orange-100 text-orange-700 text-[10px] md:text-xs font-bold uppercase tracking-widest animate-in fade-in zoom-in duration-500">
            ...
            <h1 className="text-3xl md:text-5xl lg:text-6xl font-extrabold tracking-tight text-slate-900 leading-[1.1] max-w-4xl mx-auto">
                SSC Rank Predictor <br className="hidden md:block" />
            ...
            <p className="text-sm md:text-lg text-slate-500 max-w-2xl mx-auto leading-relaxed">
                Calculate your normalized scores based on the latest SSC formulas. 
                Compare your rank with thousands of other aspirants.
            </p>
        </div>
                ...
                {exams.length === 0 && (
                    <div className="col-span-full flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-slate-200">
                        <div className="w-16 h-16 bg-orange-50 rounded-full flex items-center justify-center mb-4">
                            <BarChart3 className="text-orange-300 w-8 h-8" />
                        </div>
                        <h3 className="text-lg font-bold text-slate-700">No Active SSC Keys</h3>
                        <p className="text-slate-500 text-sm mt-1">
                            Wait for the answer key release of upcoming exams.
                        </p>
                    </div>
                )}
            </div>
...
// --- REUSED CARD COMPONENT (Identical to Landing Page) ---
    ...
    return (
        <Link href={data.url} className="block h-full group">
            <div className="relative h-full bg-white rounded-xl p-4 border border-slate-100 shadow-[0_2px_8px_-4px_rgba(0,0,0,0.05)] hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.1)] hover:-translate-y-1 transition-all duration-300 ease-out overflow-hidden flex flex-col justify-between">
                ...
                {/* Glow Effects (Customized for SSC - Orange tint) */}
                ...
                <div className="relative z-10">
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-center">
                            ...
                            <h3 className="font-bold text-slate-900 text-base leading-tight truncate pr-1 group-hover:text-orange-600 transition-colors">
                                {data.examName}
                            </h3>
                ...
                <div className="relative z-10 mt-4 pt-3 border-t border-slate-50 flex items-center justify-between">
                    <span className="text-[10px] font-bold px-2 py-1 rounded bg-orange-50 text-orange-700 uppercase tracking-wider border border-orange-100 transition-all duration-300">
                        {data.type}
                    </span>

                    <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-orange-50 text-orange-700 text-[11px] font-bold group-hover:bg-orange-600 group-hover:text-white transition-all duration-300">
                        Analyze
